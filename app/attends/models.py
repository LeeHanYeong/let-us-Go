...
class Attend(TimeStampedModel):
    STATUS_IN_PROGRESS, STATUS_SUCCEED, STATUS_FAILED = (
        "in_progress",
        "succeed",
        "failed",
    )
    ...
    status = models.CharField(
        "상태", max_length=12, default=STATUS_IN_PROGRESS, db_index=True
    )
    ...
    applicant_type = models.CharField(
        "지원자 구분",
        choices=CHOICES_APPLICANT_TYPE,
        default=APPLICANT_TYPE_NORMAL,
        max_length=10,
        db_index=True,
    )
    discount_type = models.CharField(
        "할인 구분",
        choices=CHOICES_DISCOUNT_TYPE,
        default=DISCOUNT_TYPE_NORMAL,
        max_length=10,
        db_index=True,
    )
    goods_size = models.CharField(
        "굿즈 사이즈", choices=CHOICES_SIZE, blank=True, max_length=3
    )
